...
$$\vec{E} = e_{0} e^{\pm k'' } e^{ j\omega t } \vec{e}_{y}$$
...
Alors, 
$$\vec{B} = -\frac{jk''}{\omega} (\vec{e}_{x} \wedge \underline{\overrightarrow{E}}) = \frac{k''}{\omega} E_{0} e^{ j\left( \omega t-kx- \frac{\pi}{2} \right) } \vec{e}_{z}$$