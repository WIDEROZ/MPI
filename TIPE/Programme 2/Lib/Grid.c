...
void CREATE_GRID(SDL_Renderer *renderer, const int X, const int Y){
    VERIF_SDL_COMMAND(SDL_SetRenderDrawColor(renderer, 255-color, 255-color, 255-color, SDL_ALPHA_OPAQUE), "SetRenderDrawColor");
    SDL_RenderClear(renderer);
    VERIF_SDL_COMMAND(SDL_SetRenderDrawColor(renderer, color, color, color, SDL_ALPHA_OPAQUE), "SetRenderDrawColor"); // On peut mettre 255 a la place de SDL opaque
...
void CREATE_GRID_CASE(SDL_Renderer *renderer, const int caseX, const int caseY){
    CREATE_GRID(renderer, SQUARE_WIDTH, SQUARE_WIDTH);
...
void DRAW_RECT_EVOLUTION(Var *var, const int CASE_X, const int CASE_Y, int STATE){
    ...
    */
    if(STATE == 0){
        ...
        VERIF_SDL_COMMAND(SDL_SetRenderDrawColor(var->renderer, 255-color, 255-color, 255-color, SDL_ALPHA_OPAQUE), "SetRenderDrawColor"); // On peut mettre 255 a la place de SDL opaque
    }
    else{
        // On change la couleur du rectangle en blanc
        VERIF_SDL_COMMAND(SDL_SetRenderDrawColor(var->renderer, color, color, color, SDL_ALPHA_OPAQUE), "SetRenderDrawColor"); // On peut mettre 255 a la place de SDL opaque
    }
...
void CASE_CLICK_CASE(Var *var, const int CASE_X, const int CASE_Y){
    ...
    if((0 <= CASE_X <= (var->XY_CASE_MAT->width)) && (0 <= CASE_Y <= (var->XY_CASE_MAT->height))){
        ...
        if(Case == 0){
            ...
            // On change la couleur du rectangle en blanc (ou noir suivant le background)
            VERIF_SDL_COMMAND(SDL_SetRenderDrawColor(var->renderer, color, color, color, SDL_ALPHA_OPAQUE), "SetRenderDrawColor"); // On peut mettre 255 a la place de SDL opaque
            ...
            // On change la couleur du rectangle en noir (ou blanc suivant le background)
            VERIF_SDL_COMMAND(SDL_SetRenderDrawColor(var->renderer, 255-color, 255-color, 255-color, SDL_ALPHA_OPAQUE), "SetRenderDrawColor"); // On peut mettre 255 a la place de SDL opaque
        }
...
void CASE_CLICK_CASE_DISPLAY(Var *var , const int CASE_X, const int CASE_Y){
    if (CASE_X <= CASE_NUMBER_WIDTH && CASE_Y <= CASE_NUMBER_HEIGHT){
    // On met en cible du dessin la texture
    VERIF_SDL_COMMAND(SDL_SetRenderTarget(var->renderer, var->texture), "SetRenderTarget");
    ...
    VERIF_SDL_COMMAND(SDL_SetRenderTarget(var->renderer, NULL), "SetRenderTarget");
        ...
        ExitWithError("CASE_CLICK_CASE_DISPLAY : Out of range");
    }
}
