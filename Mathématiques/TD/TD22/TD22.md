...
$$e^{ x }  \left( \frac{\partial }{\partial x}\int_{x}^{+ \infty} f(t) e^{ -t } \, dt  \right)  = -f(x)$$
$$= e^{ x }  \frac{\partial }{\partial x} \left( [f(t)e^{ -t }]_{x}^{+ \infty} + \int_{x}^{+ \infty}f'(t)e^{ -x } \, dt  \right)$$
$$\frac{\partial }{\partial x} \left( \int_{x}^{+ \infty} f(t) e^{ -t }\, dt   \right)$$
