...
Donc, 
$$\sum_{k = 1}^{+ \infty}((k+1)ka_{k+1}+(k+1)a_{k+1}+a_{k-1})x^{k} = 0$$
Comme $y(0) = 1$ alors, $a_{0} = 1$ par unicité du DSE

Ainsi, par uni. du DSE
$$\forall n \in\mathbb{N}^{*}, (k+1)^{2}a_{k+1} + a_{k-1} =0 \Rightarrow a_{k+1} = -\frac{1}{(k+1)^{2}}a_{k-1}$$
$$a_{n+1} = \frac{1}{(n+1)^{2}(n-1)^{2}}a_{n-3} = \frac{(-1)^{k}}{(n+1)^{2}(n-1)^{2}\dots 2^{2}}a_{0}$$