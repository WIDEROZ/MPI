...
$$\forall x >0, |y_{0}(x)| \leq e^{ x } \left( \int_{x}^{2x} \left| f(t)\right| e^{ -t } \, dt + \int_{2x}^{+ \infty} \left| f(t)\right|e^{ -t } \, dt \right) $$
$$ \leq  \int_{x}^{2x} \left| f(t)\right| \, dx + e^{ -x } \int_{2x}^{+ \infty} \left| f(t)\right|  \, dt \leq (1+e^{ -x }) \int_{- \infty}^{+ \infty} \left| f(t)\right| \, dt  $$
car :
$$\lim_{ n \to \infty } \sum_{k = 0}^{n} \int_{x+k}^{x+k+1} \left| f(t)\right| \, dt = \int_{x}^{+ \infty} \left| f(t) \right| \, dt $$
$$\lim_{ n \to \infty } \sum_{k = 0}^{n} \int_{x-(k+1)}^{x-k} \, dx $$
