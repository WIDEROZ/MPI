...
$$y_{0}(x) =_{x \to + \infty} O\left( \int_{x}^{+ \infty} |f(t)| \, dt \right)$$

$$\int_{x}^{+ \infty} |f(t)| \, dt = \lim_{ x_{0} \to +\infty } F(x_{0})- F(x)$$
$$\int_{0}^{+ \infty} e^{ x }(\lim_{ x_{0} \to +\infty } F(x_{0})- F(x)) \, dx = \int_{0}^{+ \infty}e^{ x }(\lim_{ x_{0} \to +\infty } F(x_{0}) - F(x)) \, dx $$
