# Exercice 2
On pose : 
$$I_{1} = ]- \infty, -1[ \text{ et } I_{2} = ]-1, 0[ \text{ et } I_{3} = ]0, + \infty[$$
Sur $I_{1}\cup I_{3}$ : 
...
$$\forall x \in I_{1} , A(x) = 2\ln(- x) - \ln(x(x+1))+\ln(2) = \ln\left( \frac{2x^{2}}{x(x+1)} \right)$$
$$\forall x \in I_{3}, A(x) = 2\ln( x) - \ln(x(x+1))+\ln(2)$$
$$\forall x \in I_{2}, A(x) = 2\ln(-x) - \ln(-x(x+1))+\ln(2)$$

Ainsi, 
$$\exists C \in \mathbb{R}, y_{H}(x) = C\frac{x}{x+1}$$
