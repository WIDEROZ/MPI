...
#### a.
On pose : 
$$y = \sum_{k =1}^{+ \infty} a_{k-1} x^{k-1} \text{ et } y'(x) = \sum_{k =1}^{+ \infty} ka_{k}x^{k-1} = \sum_{k = 0}^{+ \infty}(k+1)a_{k+1}x^{k} $$
$$y''(x) = \sum_{k=1}^{+ \infty} (k+1)k a_{k+1}x^{k-1}$$
Donc, 
$$a_{1} + \sum_{k = 1}^{+ \infty}((k+1)ka_{k+1}+(k+1)a_{k+1}+a_{k-1})x^{k} = 0$$
$$a_{1} =0$$

Ainsi, par uni. du DSE
$$\forall n \in\mathbb{N}^{*} \text{impair}, (k+1)^{2}a_{k+1} + a_{k-1} =0 \Rightarrow a_{k+1} = -\frac{1}{(k+1)^{2}}a_{k-1}$$
On pose : $N = 2n+1$ : 
$$a_{N+1} = \frac{1}{(N+1)^{2}(N-1)^{2}}a_{N-3} = \frac{(-1)^{\left\lfloor N/2+1 \right\rfloor }}{(N+1)^{2}(N-1)^{2}\dots 2^{2}}a_{0}$$
$$a_{N+1} = \frac{(-1)^{\left\lfloor N/2+1 \right\rfloor }}{\prod_{k = -1}^{\left\lfloor N/2 +1\right\rfloor }((2k+1)-N)^{2}} = \frac{(-1)^{n+1}}{\prod_{k=-1}^{n+1}(2k+1-2n+1)}$$
car $a_{0}=1$ car $y(0) = 1$
$$a_{2n+1} = \frac{(-1)^{n+1}}{2^{n+1}\prod_{k=-1}^{n+1}(k-n+1)^{2}} = \frac{1}{2^{n+1} \prod_{k=0}^{n+2}(n-k)^{2}}$$
de plus,
$$a_{2n+1} = \frac{1}{2^{n+3}(n!)^{2}}$$
