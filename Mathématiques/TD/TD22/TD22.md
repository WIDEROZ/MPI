...
Si $y$ est bornée, ie $\left|\left| y \right|\right|_{\infty} \in \mathbb{R}$, et
$$\lim_{ x \to +\infty }y(x), \lim_{ x \to -\infty } y(x)\in \mathbb{R}$$
$$y_{0}' -y_{0}+f = y' -y+f = 0 \Rightarrow y_{0}' - y_{0} = y' -y $$
$$\Rightarrow y-y_{0} = y'-y_{0}' \Rightarrow g = g'$$
On note : $g = y-y_{0}$
Donc, 
$$\exists C \in \mathbb{R}, g = C \exp$$
Alors, 
$g(x) = C e^{ x }$
