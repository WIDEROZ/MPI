...
$$=\lim_{ x_{0} \to +\infty }  \int_{x}^{x_{0}} \left| f(t)\right| \, dt  \leq \lim_{ x_{0} \to +\infty } (x_{0}- x)\left|\left| f \right|\right| _{\infty, [x, x_{0}]}  $$

Comme $f$ est intégrable sur $\mathbb{R}$, $F$ est bornée
$$\lim_{ x_{0} \to +\infty } F(x_{0}) - F(x) = $$
