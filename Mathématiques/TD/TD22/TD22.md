...
### Exercice 12
#### a.
$$\forall x \in \mathbb{R},e^{ -x } y''(x)y(x) = y^{2}(x)$$
Donc, 
$$(y^{2})'(x) = 2y(x)y'(x)  \Rightarrow y^{2}() $$
La convexité dépend du signe de $y$ si $y > 0$ $y$ est convexe sinon $y$ est concave

#### b.
Soit $y \in Sol(\mathcal{H})$ telle que $y(0) = y(1) = 0$
ie 
Si $y$ est concave sur $[0, 1]$, 
$$\forall x \in [0, 1], y(x) \leq 0$$
Comme en $0$ et en $1$ on a un point d'inflexion, 
$$$$