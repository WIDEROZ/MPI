...
#### c.
$$\left| y_{0}(x)\right| \leq e^{ x }\left( \int_{x}^{x/2} \left| f(t)\right|e^{ -t } \, dt +  \int_{x/2}^{+ \infty} \left| f(t)\right|e^{ -t } \, dx \right)  $$
...
En $+ \infty$, on a : 
$$y_{0}(x) =_{x \to + \infty} O\left( \int_{x}^{+ \infty} \left| f(t)\right| \, dt  \right)$$
