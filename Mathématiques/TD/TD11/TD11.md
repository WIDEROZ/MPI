...
symétrique ez et positive :
$$\left< A^{\top}AX, X \right> = \mathrm{Tr}(X^{\top}A^{\top}AX) = \mathrm{Tr}((AX)^{\top}I_{n}AX) \geq 0$$
$$\left|\left| A \right|\right| _{2} = \sup_{X \in \mathbb{R}^{n}, \left|\left| X \right|\right| = 1} \left|\left| AX \right|\right| $$