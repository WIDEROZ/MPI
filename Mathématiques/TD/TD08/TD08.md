...
$$\ln\left( \frac{\sqrt{n}+(-1)^{n}}{\sqrt{n+a}} \right)\sim \ln\left( \frac{\sqrt{n}+(-1)^{n}}{\sqrt{n}} \right) = \ln\left( 1+ \frac{(-1)^{n}}{\sqrt{n}} \right)$$
...
DV + CV (critère de libenitz)
Alors, 
$$\sum \left( 1+ \frac{-1}{\sqrt{n}} \right) \text{ DV}$$
Donc; 
