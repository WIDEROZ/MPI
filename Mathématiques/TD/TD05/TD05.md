...
## 3.3
### (a). 
il existe un $m \in \mathbb{N}$ tel que : $A^{m} = 0$, 
Soient $a, b, c, d \in \mathbb{C}$, tels que :
$$A = \begin{pmatrix}
a&b \\
c&d
\end{pmatrix}$$
Alors, 
$$AE_{1, 2} = \begin{pmatrix}
0&a \\
0&c
\end{pmatrix} = E_{1, 2}A = \begin{pmatrix}
c&d \\
0&0
\end{pmatrix} = \begin{pmatrix}
0&a \\
...
DEMONSTRATION PAR REC de : 
$$P_{n} : \text{"} A^{n} = \begin{pmatrix}a^{n}&na^{n-1}b\\0&a^{n}\end{pmatrix} \text{"}$$
###### Initialisation
$$A^{2} = \begin{pmatrix}
a^{2} & 2ab \\
0&a^{2}
\end{pmatrix} $$
###### Hérédité
Soit $n \in \mathbb{N}\setminus \{ 0, 1 \}$ tel que : $A^{n} = \begin{pmatrix}a^{n}&na^{n-1}b\\0&a^{n}\end{pmatrix}$
...
Alors, comme il existe $m \in \mathbb{N}$ tel que : 
$$A^{m} = \begin{pmatrix}
0&0 \\
0&0
\end{pmatrix} = \begin{pmatrix}
...
a&b \\
c&d
\end{pmatrix} = aE_{1, 1}+bE_{1, 2} + cE_{2, 1}+dE_{2, 2}$$
Alors en généralisant la démonstration précédente : 
$$\forall i, j \in [\![1, 2]\!], AE_{i, j} = 0$$
Donc, 
$$AB = A(aE_{1, 1}+bE_{1, 2} + cE_{2, 1}+dE_{2, 2})$$
$$AB = aAE_{1, 1}+bAE_{1, 2} + cAE_{2, 1}+dAE_{2, 2} = 0+0+0+0$$
Ainsi, 
$$AB = 0$$
