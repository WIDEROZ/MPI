...
## Exo 2.
### (a).
Supposons que $P \in \mathbb{C}_{p}[X]$ soit un vecteur propre de $\varphi$, il existe alors $\lambda \in \mathbb{C}$ tel que : 
$$\varphi(P) = \lambda P(X) = P(X)-(X+1)P'(X)$$
Alors, 
$$ \begin{array}{c}
(X+1)P'(X) +(\lambda-1)P(X)=0 \\
\Leftrightarrow \lambda P(X) = P(X) -(X+1)P'(X)
\end{array} $$
On pose : 
$$P(X) = \sum_{k = 0}^{d} x_{k}X^{k} \text{ alors }P'(X) = \sum_{k = 0}^{d-1}(k+1)x_{k+1}X^{k}$$
Donc, 
$$\lambda \sum_{k = 0}^{d} x_{k}X^{k} = \sum_{k = 0}^{d}x_{k}X^{k} - \sum_{k= 0}^{d-1}(k+1)x_{k+1}X^{k+1} - \sum_{k= 0}^{d-1}(k+1)x_{k+1}X^{k}$$
$$= \sum_{k = 0}^{d}(1-k)x_{k}X^{k}-\sum_{k =0}^{d-1}(k+1)x_{k+1}X^{k} =\sum_{k = 0}^{d}(1-k-kX)x_{k}X^{k}- kx_{k}X^{k-1}$$
