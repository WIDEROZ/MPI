...
Si on associe toutes les matrices de $\mathcal{M}_{n}(\mathbb{R})$ à $\mathcal{M}_{n, 1}(\mathbb{R})$
On peut essayer de trouver : $Mat_\beta(\varphi)$ :
$$Mat_{\beta}(\varphi) = \begin{pmatrix}

\end{pmatrix}$$

$$\begin{cases}
\varphi(E_{1, 1}) = A - E_{1,1} \\
\varphi(E_{2, 1}) = - E_{1, 2} \\
\dots \\
\varphi(E_{n, 1}) = - E_{1, n} \\
\varphi(E_{1, 2}) = - E_{2, 1} \\
\dots \\
\varphi(E_{1, n}) = - E_{2, n} \\
\dots \\
\varphi(E_{})
\end{cases}$$
$$\forall i, j \in $$