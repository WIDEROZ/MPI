...
$$P(X) = \prod_{k = 1}^{r}(X-\lambda_{k})^{m_{k}} $$
puis : 
$$P'(X)= \sum_{k = 1}^{r}\lambda_{k}m_{k}(X-\lambda_{k})^{m_{k}-1}\prod_{i \in [\![1, r]\!]\setminus \{ k \}} (X-\lambda_{i})^{m_{i}}$$
$$P'(X) = \sum_{k=1}^{r}\lambda_{k}m_{k} \prod_{i \in [\![1, r]\!]}(X-\lambda_{i})^{m_{i}-1}\prod_{i \in }$$
