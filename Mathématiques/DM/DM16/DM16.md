...
Si $k = 0$, 
$$\int_{-\pi}^{\pi} H(t)  \, dt = \int_{-\pi}^{\pi} \sum_{n = 0}^{+ \infty} a_{n} e^{ i(2^{n}-2^{n_{0}})t  }\, dt   \text{ comme : } \int_{-\pi}^{\pi} \left| a_{n}\right|\, dt \text{ converge, et que : }  H \text{ converge normalement donc simplement} $$
