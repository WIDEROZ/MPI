...
___
$$\int_{a}^{1} \mathrm{cotan}(\pi x) \, dx \text{ converge car : } $$
$$\frac{\cos(\pi x)}{\sin(\pi x)} \sim -\frac{1}{\pi(x-1)}$$
$$-\sin(\pi x) = \sin(\pi (x - 1)) \underset{x \to 1}{=} \pi(x-1)+ o(x)$$
___
...
$$\tilde{D}(\alpha) = \tilde{D}(\alpha + \left\lfloor 2^{n}x \right\rfloor ) = \tilde{D}\left( \frac{\alpha + \left\lfloor 2^{n}x \right\rfloor }{2^{n}} \right) = \tilde{D}\left( \frac{\alpha}{2^{n}} + \frac{\left\lfloor 2^{n}x \right\rfloor }{2^{n}} \right)$$
$$\underset{n \to +\infty}{\longrightarrow}  \tilde{D}\left( x \right)$$
$$\forall x \in [0, 1], \tilde{D}(\alpha) \underset{n \to +\infty}{\longrightarrow} \tilde{D}(x)$$


$$\frac{\left\lfloor 2^{n}x \right\rfloor }{2^{n}} \underset{n \to +\infty}{\longrightarrow}  x$$
