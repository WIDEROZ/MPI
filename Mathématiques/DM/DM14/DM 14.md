...
$$\left|\left| h_{n}\right|\right| _{\infty, \mathbb{R} \setminus \mathbb{Z}} = 2\sup_{x \in [a, b]} \left| \frac{x}{x^{2}-n^{2}}\right|$$
On a alors : 
$$h_{n}'(x) = 2\frac{}{(x^{2}-n^{2})^{2}} = -2\frac{n^{2}+x^{2}}{(x^{2}-n^{2})^{2}}\leq 0$$
ie : 
$$x^{2} \geq -\frac{n^{2}}{(x^{2}-n^{2})^{2}}$$
