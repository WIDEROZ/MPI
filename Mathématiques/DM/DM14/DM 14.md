...
$$\forall n \in \mathbb{N}^{*}, \forall x \in[-a, a], f'_{n}(x) = 2\frac{n^{2}+x^{2}}{(n^{2}-x^{2})^{2}}\geq 0$$
$$\left|\left| f_{n} \right|\right|_{\infty, [-a, a]} = 2\sup_{x \in [-a, a]} \left| \frac{x}{n^{2}-x^{2}} \right| = $$