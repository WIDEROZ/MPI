...
et sur $\mathbb{R}^{*}_{+}$ $h$ est $\mathcal{C}^{\infty}$ car $h$ est produit de fonctions $\mathcal{C}^{\infty}$ alors, 
$$\forall x \in \mathbb{R}_{+}^{*}, h'(x) = \frac{e^{ x }-1-xe^{ x }}{(e^{ x }-1)^{2}}$$
Donc, 
$$\forall x \in \mathbb{R}_{+}^{*}, h''(x)=$$




$$h^{(2n)}(x) = \left( x\mapsto x \times \frac{1}{e^{ x }-1} \right)^{(2n)}(x) = \sum_{k = 1}^{2n} \binom{2n}{k}(x\mapsto x)^{(k)}\left( x\mapsto \frac{1}{e^{ x }-1} \right)$$
