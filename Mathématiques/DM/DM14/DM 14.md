...
###### Initialisation
On calcule de taux accroissement en $0$ : 
$$\lim_{ h \to 0 } \frac{\frac{h}{e^{ h }-1}-1}{h} = \lim_{ h \to 0 } \left( \frac{1}{e^{ h }-1}- \frac{1}{h} \right) = \lim_{ h \to 0 } \left( \frac{h-e^{ h }+1}{h(e^{ h }-1)} \right)= \frac{1}{2}$$
car : 
$$\frac{h-e^{ h }-1}{h(e^{ h }-1)} \underset{h \to 0}{=} \frac{h-h-\frac{h^{2}}{2}+o(h^{2})}{h\left( h+ \frac{h^{2}}{2}+o(h^{2}) \right)} \underset{h \to 0}{=} \frac{-\frac{h^{2}}{2}+o(h^{2})}{h^{2} + \frac{h^{3}}{2} + o(h^{3})}$$
$$\underset{h \to 0}{=} \frac{-1+o(1)}{2+h+o(h)}=-\frac{1}{2}$$
Et donc : $\boxed{h'(0) = -\frac{1}{2}}$
Et puis :
$$\boxed{\forall x \in\mathbb{R}^{*}, h'(x) =\frac{e^{ x }-1 - xe^{ x }}{(e^{ x }-1)^{2}}}$$
$$\lim_{ x \to 0 } h'(x) = -\frac{1}{2} $$
car : 
$$\frac{1+x+\frac{x^{2}}{2}+o(x^{2})-1 - x(1+x+ o(x))}{\left( 1+x+\frac{x^{2}}{2}+o(x^{2})-1 \right)^{2}}= \frac{-\frac{x^{2}}{2} + o(x^{2})}{\left( x + \frac{x^{2}}{2} + o(x^{2}) \right)^{2}}$$
$$= \frac{-1 + o(x^{2})}{\left( \sqrt{2}+\frac{x}{\sqrt{2}} + o(x) \right)^{2}} \underset{x \to 0}{\longrightarrow} -\frac{1}{2}$$

Fonctionne bien pour la formule car $\zeta(2) = \frac{\pi^{2}}{6}$

de même on obtiens : 
$$h''(x) = \frac{-3xe^{ 2x }+xe^{ x }-2e^{ 2x }-2e^{ x }}{(e^{ x }-1)^{3}}$$