...
de plus, 
$$\forall p \in [\![0, 2n-1]\!], \sum_{n \geq 1} g_{n}^{(p)}(x)\text{ converge simplement sur } \mathbb{R}$$
Alors, 
$$G^{(p)}(x) = \sum_{n = \left\lfloor \frac{p}{2} \right\rfloor }^{+ \infty} g^{(p)}_{n}(x) = \sum_{n = 0}^{+ \infty} \frac{(-1)^{n-1} \zeta(2n)(2n)!}{2^{2n-1}\pi^{2n}(2n-p)!} x^{2n}$$
$n = \frac{p}{2}$ => $n=0$



$$\left| \frac{g_{n+1}^{(p)}(x)}{g_{n}^{(p)}(x)}\right| = \frac{\frac{\zeta(2n+1)}{\zeta(2n)}(2n+1)}{4\pi^{2}(2n+1-p)}x^{2} \underset{n \to +\infty}{\longrightarrow} $$
$$\sum_{k = 1}^{+ \infty} \frac{1}{k^{2n}} = \sum_{k = 1}^{+ \infty} \frac{1}{k^{2n}} $$
