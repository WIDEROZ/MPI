nce de la limite des sommes partielles dans l'expressio#### Note
...
de plus, 
$$\forall p \in [\![0, 2n-1]\!], \sum_{n \geq 0} g_{n}^{(p)}\text{ converge simplement sur } \mathbb{R} \text{ ATTENTION tentative de GRUGE}$$
Alors, 
$$\forall x \in \mathbb{R}, G^{(p)}(x) = \sum_{n = \left\lfloor \frac{p}{2} \right\rfloor }^{+ \infty} g^{(p)}_{n}(x) = \sum_{n = 0}^{+ \infty} \frac{(-1)^{n-1} \zeta(2n)(2n)!}{2^{2n-1}\pi^{2n}} x^{2n}$$
par un changement de variable dans la somme. 
___
Montrons que : 
$$\sum_{n \geq 0} g_{n}^{(2n)} \text{ converge uniformément sur }\mathbb{R}$$
$$g^{(2n)}(x) = \frac{(-1)^{n-1} \zeta(2n)(2n)!}{2^{2n-1}\pi^{2n}}x$$
Montrons que la série converge normalement sur tout segment : 
Soit $[a, b] \subset \mathbb{R}$, ou $a<b$
$$\left|\left| g^{(2n)} \right|\right|_{\infty} = \sup_{x \in [a, b]}| g_{n}(x)| = \frac{\zeta(2n)(2n)!}{2^{2n-1}\pi^{2n}}b$$
$$\sum_{n = 1}^{+ \infty} \frac{\zeta(2n) (2n!)}{2^{2n-1}\pi^{2n}}b$$

$$\frac{1}{k^{2n+1}} =_{n \to + \infty} O\left( \frac{1}{k^{2n}} \right)$$
Alors, 
$$\sum_{k = n+1}^{+ \infty} \frac{1}{k^{2n+1}} =_{n \to +\infty} O\left( \sum_{k = n+1}^{+ \infty}\frac{1}{k^{2n}} \right)$$
