...
de plus, 
$$\forall p \in [\![0, 2n-1]\!], \sum_{n \geq 1} g_{n}^{(p)}(x)\text{ converge sur } \mathbb{R} \text{ car la série des } g_{n}^{(p)}(x) = \frac{(-1)^{n-1} \zeta(2n)(2n)!}{2^{2n-1}\pi^{2n}(2n-p)!} x^{2n-p} \text{ converge : }$$
