#### Note
Les petits points corespondent à des questions laissés sans finalité. 
...
### Question 7
La fonction est bien continue car : 
$$\lim_{ x \to 0 } \frac{x}{e^{ x }-1} = \lim_{ x \to 0 } \frac{x}{x + o(x)} = 1$$
elle est aussi dérivable en 0 car : 
$$h'(0) = \lim_{ h \to 0 } \frac{h(h)-h(0)}{h} = \lim_{ h \to 0 } \frac{\frac{h}{e^{ h }-1} - 1}{h} = \lim_{ h \to 0 } \left( \frac{h-e^{ h }+1}{h(e^{ h }-1)} \right)=\lim_{ h \to 0 } \frac{-\frac{h^{2}}{2}+ o(h^{2})}{h^{2} + o(h^{2})} = \lim_{ h \to 0 } \frac{-\frac{1}{2}+o(1)}{1+o(1)} = -\frac{1}{2}$$
et sur $\mathbb{R}^{*}$ $h$ est $\mathcal{C}^{\infty}$ car $h$ est produit de fonctions $\mathcal{C}^{\infty}$ alors, 
$$\forall x \in \mathbb{R}_{+}^{*}, h'(x) = \frac{e^{ x }-1-xe^{ x }}{(e^{ x }-1)^{2}}$$
Donc, 
$$\forall x \in \mathbb{R}^{*}, h''(x) = \frac{e^{ x }-(e^{ x }-xe^{ x })(e^{ x }-1)-(e^{ x }-1-xe^{ x })2e^{ x }}{(e^{ x }-1)^{3}} = \frac{2e^{ x }-e^{ 2x }+xe^{ 2x }-xe^{ x }-2e^{ 2x } + 2e^{ x }+ 2x e^{ 2x }}{(e^{ x }-1)^{3}}= \frac{4-3e^{ x }-x+3xe^{ x }}{(e^{ x }-1)^{3}}e^{ x } $$
et
$$h''(0) = \lim_{ h \to 0 } \frac{h'(h) +\frac{1}{2}}{h} = \lim_{ h \to 0 } \frac{e^{ h }-1-he^{ h }+\frac{1}{2}(e^{ h }-1)^{2}}{(e^{ h }-1)^{2}h} = \lim_{ h \to 0 } \frac{1+h+\frac{h^{2}}{2}-1 - h-h^{2}-\frac{h^{3}}{2}+ \frac{1}{2}\left( h+\frac{h^{2}}{2} + o(h^{2})\right)^{2}+ o(h^{3})}{\left( h+\frac{h^{2}}{2}+o(h^{2}) \right)^{2}h} $$
$$= \frac{-\frac{h^{2}}{2} -\frac{h^{3}}{2} + \frac{1}{2}h^{2} + \frac{1}{2}h^{3}+ o(h^{3})}{h^{2}+h^{3} + o(h^{3})} = \frac{-\frac{1}{2}h^{3} + o(h^{3})}{h^{2}+\frac{h^{3}}{2}+o(h^{3})} $$
 




$$\frac{h-h-h^{2}+ \frac{1}{2}h^{2}}{h^{3}}$$
$$h^{(2n)}(x) = \left( x\mapsto x \times \frac{1}{e^{ x }-1} \right)^{(2n)}(x) = \sum_{k = 0}^{2n} \binom{2n}{k}(x\mapsto x)^{(k)}(x)\left( x\mapsto \frac{1}{e^{ x }-1} \right)^{(2n-k)}(x) = \sum_{k = 1}^{2n} \frac{(2n)!}{k!(2n-k)!} \times $$




$$\frac{z}{e^{ z }-1} = 1-\frac{z}{2} -\sum_{k = 1}^{+ \infty} \frac{(-1)^{k-1} \zeta(2k)}{2^{2k-1}\pi^{2k}}z^{2k}$$
