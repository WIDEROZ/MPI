...
#### e.
$$\sum_{n = 1}^{+ \infty}v_{n}(1) = \sum_{n = 1}^{+ \infty} \left( \frac{1}{n^{x}} - \ln\left(  1 + \frac{1}{n} \right) \right) \underset{x \to 1}{=} \sum_{n = 1}^{+ \infty} \frac{1}{2n^{2}}$$

D'après la question 5.c : 
$$\forall x \in ]1, 2], Z(x) = \sum_{n=1}^{+ \infty} v_{n}(x) + \frac{1}{x-1}$$
Or comme $\sum_{n\in \mathbb{N}^{*}} v_{n}$ converge uniformément, par le ethéorème de la double limite :
